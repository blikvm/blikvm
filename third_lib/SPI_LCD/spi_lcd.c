...
static struct gpiod_line *lines[512];	  // GPIO 引脚
...
void myPinWrite(int iPin, int iValue)
{
	...
	if (!lines[iPin])
	{
		lines[iPin] = gpiod_chip_get_line(chip, iPin); // 获取 GPIO 引脚
		if (!lines[iPin])
		{
			fprintf(stderr, "myPinWrite Error getting GPIO line %d\n", iPin);
			return;
		}

		if (gpiod_line_request_output(lines[iPin], "spi_lcd", 0) < 0)
		{
...
void GenericAddGPIO(int iPin, int iDirection, int bPullup) {
    ...
    if (iDirection == GPIO_OUT) {
        ...
        if (gpiod_line_request_input(lines[iPin], "spi_lcd") < 0) {
...
int spilcdWriteStringFast(int x, int y, char *szMsg, unsigned short usFGColor, unsigned short usBGColor)
{
	...
	if ((8 * iLen) + x > iMaxLen)
		iLen = (iMaxLen - x) / 8;				 // can't display it all
