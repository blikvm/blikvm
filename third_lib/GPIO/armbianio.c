...
static int iOPI1Pins[] = {355, -1, -1, 12, -1, 11, -1, 6, 13, -1,
						  ...
						  -1, 8, 200, 9, -1, 10, 201, 20, 198, -1, 199, 4, 5, -1}; // last 3 pins are TTY header
...
void *GPIOIRThread(void *param)
{
	...
	while (1)
	{
		...
		if (fdset[0].revents & POLLPRI)
		{
			...
			if (codePointer > 50)
			{ // max code length reached, send what we have.
				if (cbIRList[iPin])
		...
		else if (timeout == endOfCodeTimeOut)
		{ // are we receiving a code currently?
